...
struct HandymanServicesView: View {
    @State private var recentBookings: [HandymanBooking] = []
    @State private var pastOrders: [HandymanOrder] = []
    ...
    var body: some View {
                    ...
                    if recentBookings.isEmpty {
                        ...
                        LazyVStack(spacing: 0) {
                            ForEach(recentBookings) { booking in
                                HandymanBookingItemView(booking: booking)
                                    .onTapGesture {
                                        // Navigate to HandymanBookingDetailsView
                                    }
                            }
                        }
                        .background(Color.gray.opacity(0.05))
                    ...
                    if pastOrders.isEmpty {
                        ...
                        LazyVStack(spacing: 0) {
                            ForEach(pastOrders) { order in
                                HandymanOrderItemView(order: order)
                                    .onTapGesture {
                                        // Navigate to HandymanOrderDetailsView
                                    }
                            }
                        }
                        .background(Color.gray.opacity(0.05))
        ...
        .navigationTitle("All Handyman Services")
    ...
    private func fetchRecentBookings() {
        ...
        NetworkManager.shared.postRequest(
            url: APIClient.baseUrl + "customers_all_handyman_bookings",
    ...
    private func fetchPastOrders() {
        ...
        NetworkManager.shared.postRequest(
            url: APIClient.baseUrl + "customers_all_handyman_orders",
            parameters: parameters
        ) { (result: Result<[String: Any], NetworkError>) in
            showLoading = false
            switch result {
...
struct HandymanBookingItemView: View {
    let booking: HandymanBooking
...
struct HandymanOrderItemView: View {
    let order: HandymanOrder
